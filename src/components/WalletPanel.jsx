...
const WalletPanel = ({ onClose }) => {
  ...
  const handleAddCard = () => {
    ...
    if (cleanNum.length !== requiredLength) {
      setErrorMsg('Número de tarjeta incompleto.');
      return;
    }
    
    if (expiry.length < 5) {
      setErrorMsg('Fecha de expiración incompleta.');
      return;
    }
    ...
    if (expM < 1 || expM > 12) {
      setErrorMsg('Mes de expiración inválido.');
      return;
    }
    ...
    if (cvv.length !== requiredCvv) {
      setErrorMsg(`El código de seguridad debe ser de ${requiredCvv} dígitos.`);
      return;
    }
          ...
          <div className="flex flex-col gap-3">
            ...
            {!isAdding ? (
              ...
              <div className="bg-[#F3F4F6] p-5 rounded-2xl flex flex-col gap-3 animate-fade-in mt-2 border-none">
                ...
                {errorMsg && (
                  <span className="text-[13px] text-[#FF3B30] font-medium px-1 animate-fade-in">{errorMsg}</span>
                )}
