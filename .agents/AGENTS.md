...
### Disposición Estándar de Paneles de Administración y Contenedores Deslizables (Edge-to-Edge Chips)
- **Estructura Estándar de Paneles Admin:** Todas las secciones del panel de administración deben compartir la misma estructura visual y espaciado vertical idéntico: encabezado con título/descripción y botón de acción en un contenedor con `gap-4` respecto a la barra de búsqueda y filtros, seguido por la tarjeta principal de contenido en `bg-white rounded-2xl flex-1 p-2 md:p-4`. Queda prohibido añadir paddings extras (`p-4` o `p-6`) entre el encabezado y el buscador.
- **Desplazamiento Edge-to-Edge para Chips/Filtros:** Los contenedores horizontales de chips/filtros deslizables deben usar márgenes negativos equivalentes al padding del contenedor padre (ej. `-mx-4 px-4 md:-mx-8 md:px-8 flex overflow-x-auto no-scrollbar`). Esto garantiza que al deslizarse no existan márgenes blancos ni recortes laterales artificiales, permitiendo un flujo continuo de borde a borde.
